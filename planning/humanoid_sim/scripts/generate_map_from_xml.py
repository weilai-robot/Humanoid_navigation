...
def write_pgm(path, grid):
    """写 P5 PGM。行序必须 row0=世界最大 Y (top-down), 这是 PGM/map_server
    约定; grid[j][i] 的 j 从 MAP_Y0 (最小 Y) 递增, 故写出时反转行序。
    (CI run 33036512784..33045503181: A/B/D 全部 planner 拒绝的根因即
    此翻转缺失 — 通道A/通道B 在地图上互换位置, X=2 以东目标不可达)"""
    ...
    with open(path, 'wb') as f:
        ...
        f.write(bytes(PGM_OCC if grid[j][i] else PGM_FREE
                      for j in range(h - 1, -1, -1) for i in range(w)))
